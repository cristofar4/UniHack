...
app.post('/api/waitlist', async (req, res) => {
  ...
  try {
    ...
    if (result.rowCount === 0) return res.json({ success: true, message: 'Already joined', count: total });
...
// Keep-alive so Render's free tier doesn't sleep
setInterval(() => {
...
app.listen(PORT, () => console.log(`🚀 UniHack server running on port ${PORT}`));
